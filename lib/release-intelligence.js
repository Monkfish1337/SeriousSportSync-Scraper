...
function emptyState() { return { version: 1, updatedAt: null, lastCollection: null, items: [] }; }
function load() {
  try {
    if (!fs.existsSync(FILE)) return emptyState();
    const parsed = JSON.parse(fs.readFileSync(FILE, 'utf8'));
    if (!parsed || !Array.isArray(parsed.items)) return emptyState();
    parsed.items = parsed.items.map((item) => Object.assign({}, item,
...
// An unknown publish date must stay unknown. `new Date(value || 0)` turned
// null/''/0 into the epoch, which then passed the year guard, so every item
// with no date came back from disk stamped 1970-01-01 — a date that sorts and
// displays as if it were real. Absent input is rejected before parsing; the
// year guard is only there to drop upstream placeholders like 0001-01-01.
...
function ingest(observations, source) {
  ...
  save({ version: 1, updatedAt: now, lastCollection: state.lastCollection || null, items });
...
function pruneSourceCategories(source) {
  ...
  for (const item of state.items) {
    ...
    const accepted = categories.has('title-verified-sport')
      || Array.from(allowed).some((value) => categories.has(value));
  ...
  if (removedOrigins || removedItems) save({ version: 1, updatedAt: state.updatedAt,
    lastCollection: state.lastCollection || null, items });
  ...
  activeRun = (async () => {
    ...
    for (const source of settings.enabledSources()) {
      ...
      try {
        ...
        const details = Array.isArray(rows && rows.diagnostics) ? rows.diagnostics.map((detail) => ({
          id: cleanText(detail.id, 40), name: cleanText(detail.name, 100),
          protocol: cleanText(detail.protocol, 20), state: cleanText(detail.state, 30),
          mode: cleanText(detail.mode, 40), raw: Math.max(0, Number(detail.raw) || 0),
          feedRaw: Math.max(0, Number(detail.feedRaw) || 0),
          searchRaw: Math.max(0, Number(detail.searchRaw) || 0),
          titleVerified: Math.max(0, Number(detail.titleVerified) || 0),
          accepted: Math.max(0, Number(detail.accepted) || 0),
          rejected: Math.max(0, Number(detail.rejected) || 0),
          durationMs: Math.max(0, Number(detail.durationMs) || 0),
          queries: (Array.isArray(detail.queries) ? detail.queries : []).map((value) => cleanText(value, 100)).slice(0, 3),
        ...
        outcomes.push({ sourceId: source.id, name: source.name, supported: true,
          ...
          removed: pruned.removedItems, total: result.total, details });
      } catch (error) {
...
function status() {
  const state = load();
  return { running: !!activeRun, lastRun: lastRun || state.lastCollection || null, stats: stats() };
}
