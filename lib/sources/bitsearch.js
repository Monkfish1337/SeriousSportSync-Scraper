...
// bitsearch.eu is a free public endpoint with no API key, so the only thing
// keeping us welcome is not hammering it. A multi-title search fires one
// request per title back to back, and the whole companion shares one egress IP
// through gluetun — so several sources searching at once look, from their side,
// like one very busy client. This is process-wide on purpose: per-call spacing
// would not help when three searches run concurrently.
let nextAllowedAt = 0;
async function paceRequest(minIntervalMs) {
...
// Retry-After is seconds or an HTTP date. Cap it: a host asking us to wait a
// minute is telling us to back off this run, not to hold a request open.
function retryAfterMs(header, capMs) {
  if (!header) return 0;
...
async function fetchSearch(query, sourceConfig, log) {
  ...
  const minIntervalMs = cfg.minIntervalMs === undefined ? 1500 : Number(cfg.minIntervalMs) || 0;
  const backoffCapMs = Math.max(1000, Number(cfg.timeoutMs) || 10000);
  let res;
  let body = '';

  // One retry, and only for the statuses that mean "later, not never".
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (attempt > 0) break;
    const waitMs = retryAfterMs(res.headers.get('retry-after'), backoffCapMs)
      || Math.max(2000, minIntervalMs * 2);
    log.warn('source', 'bitsearch HTTP ' + res.status + ' (rate limited) — waiting '
      + waitMs + 'ms and retrying once');
    await delay(waitMs);
  }

  if (res.status === 429 || res.status === 503) {
    // Still limited after backing off. Say so precisely: this is not a broken
    // source and not an empty index, it is too much traffic from this IP.
    const reason = 'HTTP ' + res.status + ' — rate limited by bitsearch.eu. '
      + 'Raise "Min interval between requests", search fewer titles, or give it a rest.';
    log.warn('source', 'bitsearch ' + reason);
    return { ok: false, reason, rows: [], raw: 0, dropped: 0, rateLimited: true };
  }
...
async function test(sourceConfig, log) {
  ...
  if (!result.ok) {
    // A rate limit is not "unreachable" — the host answered, it just said not
    // now. Calling it unreachable sends you looking at DNS and the VPN.
    return {
      ok: false,
      latencyMs,
      message: (result.rateLimited ? 'bitsearch ' : 'bitsearch unreachable — ') + result.reason,
    };
  }
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'minIntervalMs', label: 'Min interval between requests (ms)', type: 'number', default: 1500,
      hint: 'bitsearch.eu is a free endpoint with no key. Raise this if you see HTTP 429.' },
  ...
  _test: {
    endpointUrl, fetchSearch, normHash, retryAfterMs,
    resetPacing() { nextAllowedAt = 0; },
  },
};
