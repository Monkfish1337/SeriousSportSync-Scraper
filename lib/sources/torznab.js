  ...
  while ((m = itemRe.exec(xml))) {
    ...
    if (!requireHash) {
      out.push({ title, size, publishedAt: publishDate,
        ...
        categories: [attrs.category].filter(Boolean) });
      continue;
    }
    if (!hash) continue;
...
async function recent(sourceConfig, log) {
  ...
  if (sourceConfig.apiKey) params.set('apikey', sourceConfig.apiKey);
  const cats = (sourceConfig.intelligenceCategories || '5060,5000').split(',').map((value) => value.trim()).filter(Boolean);
  if (cats.length) params.set('cat', cats.join(','));
  const url = endpointUrl(sourceConfig) + '?' + params.toString();
  let response;
  try {
  ...
  if (!response.ok) { log.warn('intelligence', 'torznab recent feed HTTP ' + response.status); return []; }
  return parseItems(await response.text(), sourceConfig, false);
}
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'intelligenceCategories', label: 'Recent-feed categories', type: 'csv', default: '5060,5000', hint: 'Naming collector only. TV/Sport plus TV fallback by default.' },
