...
async function singleSearch(query, sourceConfig, log, offset) {
...
// Queries used to run one after another. With five aliases against a slow
// indexer that is five serial round trips inside a fixed SCRAPE_BUDGET_MS, so
// the later aliases were routinely never issued at all — a pull-rate loss that
// looked like "the release isn't indexed". Prowlarr already pools its queries;
// this brings Torznab (and therefore Bitmagnet) in line.
//
// `pages` walks the offset window for indexers that support it. A query whose
// first page comes back exactly full is truncated by definition, and the rows
// you want may be the ones that did not fit — see the EPL corpus, where a
// broad query had ~1,146 genuine hits and limit=100 surfaced 0.6% of them.
async function multiSearch(queries, sourceConfig, log) {
  const label = sourceConfig.label || 'torznab';
  const unique = Array.from(new Set((queries || [])
    .map((q) => String(q || '').trim()).filter(Boolean)));
  const limit = Number(sourceConfig.limit) || 100;
  const maxPages = Math.max(1, Math.min(10, Number(sourceConfig.maxPages) || 1));
  const collected = [];
  let truncated = false;
  ...
  async function worker() {
    while (true) {
      ...
      if (sourceConfig._requestDeadlineAt && Date.now() >= sourceConfig._requestDeadlineAt) {
        log.warn('source', label + ' deadline reached; ' + (unique.length - index)
          + ' query/queries not issued');
        return;
      }
      const q = unique[index];
      log.info('source', label + ' query "' + q + '"');
      let total = 0;
      for (let page = 0; page < maxPages; page++) {
        const results = await singleSearch(q, sourceConfig, log, page * limit);
        total += results.length;
        for (const r of results) collected.push(r);
        if (results.length < limit) break;
        truncated = true;
        if (sourceConfig._requestDeadlineAt && Date.now() >= sourceConfig._requestDeadlineAt) break;
      }
      log.info('source', '  -> ' + total + ' hits' + (total >= limit ? ' (page-limited)' : ''));
    }
  }
  ...
  if (truncated) {
    log.warn('source', label + ' hit the per-query limit of ' + limit
      + ' — results are truncated, raise "Limit per query" or "Max pages"');
  }
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'maxPages',   label: 'Max pages per query', type: 'number', default: 1,
      hint: 'Walks the offset window when a query fills its limit. Raise for local sources such as Bitmagnet where over-fetching is cheap.' },
    { name: 'queryConcurrency', label: 'Concurrent queries', type: 'number', default: 3,
      hint: 'Aliases are searched in parallel up to this many at a time.' },
