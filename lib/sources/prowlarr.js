...
async function recent(sourceConfig, log) {
  ...
  const feedConfig = Object.assign({}, sourceConfig, {
    categories: sourceConfig.intelligenceCategories || '5060,5000',
    limit: Math.max(1, Math.min(500, Number(sourceConfig.intelligenceLimit) || 250)),
  });
  const results = await singleSearch('', feedConfig, log);
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'intelligenceCategories', label: 'Recent-feed categories', type: 'csv', default: '5060,5000', hint: 'Naming collector only. TV/Sport plus TV fallback by default; adjust to your indexer mapping.' },
