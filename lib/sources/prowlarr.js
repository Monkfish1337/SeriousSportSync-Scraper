...
async function singleSearch(query, sourceConfig, log, options) {
  ...
  const context = options && options.context === 'search' ? 'search' : 'recent feed';
  ...
  const cats = sourceConfig._omitCategories ? []
    : (sourceConfig.categories || '2000,5000,8000').split(',').map((s) => s.trim()).filter(Boolean);
  ...
  try {
    ...
    if (strict) throw new Error('Prowlarr ' + context + ' network error');
    return [];
  }
  if (!res.ok) {
    log.warn('source', 'prowlarr HTTP ' + res.status);
    if (strict) throw new Error('Prowlarr ' + context + ' HTTP ' + res.status);
    return [];
  }
  let body;
  try { body = await res.json(); }
    ...
    if (strict) throw new Error('Prowlarr ' + context + ' returned invalid JSON');
    return [];
  }
...
function fallbackQueries(sourceConfig, indexerId, now) {
  const configured = String(sourceConfig.intelligenceFallbackQueries
    || 'UFC;ONE Friday Fights;ONE Fight Night;WWE;AEW;Formula 1;MLB;Champions League')
...
function matchesFallbackTitle(title, query) {
  const actual = new Set(titleTokens(title));
  const wanted = Array.from(new Set(titleTokens(query)
    .filter((token) => token.length >= 2 || /^\d+$/.test(token))));
  if (wanted.length > 0 && wanted.every((token) => actual.has(token))) return true;
  const phrase = wanted.join(' ');
  const verifiedAliases = {
    'formula 1': [['f1'], ['formula1']],
    'champions league': [['ucl']],
    'uefa champions league': [['ucl']],
    'one friday fights': [['one', 'ff']],
    'one fight night': [['one', 'fn']],
  };
  return (verifiedAliases[phrase] || []).some((variant) =>
    variant.every((token) => actual.has(token)));
}

function looksLikeGameRelease(title) {
  return /^\s*[\[(](?:psp|ps[1-5]|playstation(?:\s*[1-5])?|xbox(?:\s*(?:360|one))?|wiiu?|switch|nintendo(?:\s+(?:ds|3ds|switch))?|pc)[\])]/i
    .test(String(title || ''));
}

function normalizeIntelligenceRows(results, indexer, requested, query) {
  const accepted = [];
  for (const result of results || []) {
    const title = String(result.title || '').trim();
    if (!title) continue;
    const categoryMatch = requested.some((category) => resultCategoryIds(result).includes(category));
    const titleVerified = !categoryMatch && !!query && matchesFallbackTitle(title, query);
    if (!categoryMatch && !titleVerified) continue;
    if (titleVerified && looksLikeGameRelease(title)) continue;
    const categories = Array.isArray(result.categories) ? result.categories.slice() : [];
    if (titleVerified) categories.push('title-verified-sport');
    accepted.push({
...
async function recent(sourceConfig, log) {
  ...
  const feedConfig = Object.assign({}, sourceConfig, {
    ...
    timeoutMs: Math.max(1000, Math.min(120000,
      Number(sourceConfig.intelligenceTimeoutMs) || 30000)),
  });
  let indexers;
  try { indexers = await listIndexers(feedConfig); }
  catch (error) {
  ...
  for (const indexer of indexers.filter((entry) => entry && entry.enable === false)) {
    diagnostics.push({ id: indexer.id, name: indexer.name || 'Indexer ' + indexer.id,
      ...
      feedRaw: 0, searchRaw: 0, titleVerified: 0, accepted: 0, rejected: 0,
      durationMs: 0, reason: 'Disabled in Prowlarr' });
  }
  let next = 0;
  async function worker() {
    while (true) {
      ...
      const detail = { id: indexer.id, name: indexer.name || 'Indexer ' + indexer.id,
        ...
        raw: 0, feedRaw: 0, searchRaw: 0, titleVerified: 0,
        accepted: 0, rejected: 0, durationMs: 0, reason: '' };
      const supported = categoryIds(indexer.capabilities && indexer.capabilities.categories);
      const categorySupported = !supported.length
        || requested.some((category) => supported.includes(category));
      ...
      try {
        ...
        if (!rows.length && indexer.supportsSearch !== false) {
          ...
          for (const query of queries) {
            results = await singleSearch(query, oneConfig, log,
              { throwOnError: true, context: 'search' });
            const queryRows = normalizeIntelligenceRows(results, indexer, requested, query);
            detail.searchRaw += results.length;
            detail.titleVerified += queryRows.filter((row) =>
              row.categories.includes('title-verified-sport')).length;
            detail.raw += results.length;
        ...
        detail.reason = rows.length ? (detail.titleVerified
          ? detail.titleVerified + ' title(s) verified from explicit sports searches' : '')
          : (detail.mode === 'rotating search'
          ? detail.raw + ' result(s) returned; none matched category '
            + requested.join(', ') + ' or the requested sports title'
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'intelligenceTimeoutMs', label: 'Indexer collection timeout (ms)', type: 'number', default: 30000, hint: 'Release Intelligence only. Raise this for slow indexers without increasing playback search time.' },
    ...
    { name: 'intelligenceFallbackQueries', label: 'Rotating fallback searches', type: 'text', default: 'UFC;ONE Friday Fights;ONE Fight Night;WWE;AEW;Formula 1;MLB;Champions League', hint: 'Semicolon-separated. Used only when an indexer returns no recent sport titles.' },
  ...
  categoryIds,
  fallbackQueries,
  matchesFallbackTitle,
  looksLikeGameRelease,
};
