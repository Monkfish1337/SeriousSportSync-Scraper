...
async function recent(sourceConfig, log) {
  log.info('intelligence', 'reading recent Prowlarr titles');
  ...
  const feedConfig = Object.assign({}, sourceConfig, {
    ...
    limit: Math.max(1, Math.min(500, Number(sourceConfig.intelligenceLimit) || 250)),
  });
  const results = await singleSearch('', feedConfig, log, { throwOnError: true });
  const filtered = results.filter((result) => {
    const observed = (Array.isArray(result.categories) ? result.categories : [])
      .map((category) => String(category && (category.id || category.name) || category));
    return requested.some((category) => observed.includes(category));
  });
  if (results.length !== filtered.length) log.info('intelligence', 'discarded '
    + (results.length - filtered.length) + ' title(s) outside configured sport categories');
  return filtered.map((result) => ({
    title: String(result.title || '').trim(),
    size: Number(result.size) || 0,
    publishedAt: result.publishDate || null,
    indexer: result.indexer || 'Prowlarr',
    protocol: /usenet/i.test(String(result.protocol || '')) ? 'usenet' : 'torrent',
...
module.exports = {
  ...
  schema: [
    ...
    { name: 'intelligenceLimit', label: 'Recent titles per collection', type: 'number', default: 250, hint: 'Naming collector only; one request per collection interval.' },
  ],
